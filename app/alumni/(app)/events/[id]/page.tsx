...
export default function AlumniEventDetailPage() {
  ...
  return (
    <div>
      ...
      {images[0] ? (
        <div className="alumni-detail-hero">
          <img src={images[0]} alt={event.title} />
          <div className="alumni-detail-hero-overlay">
            <h1>{event.title}</h1>
          </div>
        </div>
      ) : (
        <h1 className="alumni-page-title">{event.title}</h1>
      )}
      ...
      <div className="alumni-card alumni-detail-panel">
        ...
        <p className="alumni-detail-body">{event.description}</p>

        <div className="alumni-action-row">
          {canRegister && (
            registered ? (
              <button className="alumni-btn alumni-btn-success" disabled>
                You are registered
              </button>
            ) : (
              <button className="alumni-btn alumni-btn-primary" onClick={() => setShowRegister(true)}>
      ...
      {images.length > 0 && (
        <div className="alumni-card" style={{ marginTop: 16 }}>
          <div className="alumni-section-title">Event gallery</div>
